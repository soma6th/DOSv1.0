 ...
 * @return False if connection is valid, True otherwise
 */
int HMC5883L::testConnection() {
    wiringPiI2CReadBytes(fd, HMC5883L_ADDRESS, HMC5883L_RA_ID_A, 3, buffer);
    buffer[3] = 0;
    if(!strcmp((char*)buffer, "H43"))
    {
    	return 0; //OK!
    }
    return 1; //Error!
...
uint8_t HMC5883L::getSampleAveraging() {
    wiringPiI2CReadBits(fd, HMC5883L_ADDRESS, HMC5883L_RA_CONFIG_A, HMC5883L_CRA_AVERAGE_BIT, HMC5883L_CRA_AVERAGE_LENGTH, buffer);
    return buffer[0];
}
...
void HMC5883L::setSampleAveraging(uint8_t averaging) {
    wiringPiI2CWriteBits(fd, HMC5883L_ADDRESS, HMC5883L_RA_CONFIG_A, HMC5883L_CRA_AVERAGE_BIT, HMC5883L_CRA_AVERAGE_LENGTH, averaging);
}
...
uint8_t HMC5883L::getDataRate() {
    wiringPiI2CReadBits(fd, HMC5883L_ADDRESS, HMC5883L_RA_CONFIG_A, HMC5883L_CRA_RATE_BIT, HMC5883L_CRA_RATE_LENGTH, buffer);
    return buffer[0];
}
...
void HMC5883L::setDataRate(uint8_t rate) {
	wiringPiI2CWriteBits(fd, HMC5883L_ADDRESS, HMC5883L_RA_CONFIG_A, HMC5883L_CRA_RATE_BIT, HMC5883L_CRA_RATE_LENGTH, rate);
}
...
uint8_t HMC5883L::getMeasurementBias() {
	wiringPiI2CReadBits(fd, HMC5883L_ADDRESS, HMC5883L_RA_CONFIG_A, HMC5883L_CRA_BIAS_BIT, HMC5883L_CRA_BIAS_LENGTH, buffer);
    return buffer[0];
}
...
void HMC5883L::setMeasurementBias(uint8_t bias) {
	wiringPiI2CWriteBits(fd, HMC5883L_ADDRESS, HMC5883L_RA_CONFIG_A, HMC5883L_CRA_BIAS_BIT, HMC5883L_CRA_BIAS_LENGTH, bias);
}
...
uint8_t HMC5883L::getGain() {
	wiringPiI2CReadBits(fd, HMC5883L_ADDRESS, HMC5883L_RA_CONFIG_B, HMC5883L_CRB_GAIN_BIT, HMC5883L_CRB_GAIN_LENGTH, buffer);
    return buffer[0];
}
...
void HMC5883L::setGain(uint8_t gain) {
    ...
    wiringPiI2CWriteByte(fd, HMC5883L_ADDRESS, HMC5883L_RA_CONFIG_B, gain << (HMC5883L_CRB_GAIN_BIT - HMC5883L_CRB_GAIN_LENGTH + 1));
}
...
uint8_t HMC5883L::getMode() {
	wiringPiI2CReadBits(fd, HMC5883L_ADDRESS, HMC5883L_RA_MODE, HMC5883L_MODEREG_BIT, HMC5883L_MODEREG_LENGTH, buffer);
    return buffer[0];
}
...
void HMC5883L::setMode(uint8_t newMode) {
    ...
    // requirement specified in the datasheet; it's actually more efficient than
    // using the I2Cdev.writeBits method
	wiringPiI2CWriteByte(fd, HMC5883L_ADDRESS, HMC5883L_RA_MODE, newMode << (HMC5883L_MODEREG_BIT - HMC5883L_MODEREG_LENGTH + 1));
...
void HMC5883L::getHeading(int16_t *x, int16_t *y, int16_t *z) {
	while(!getReadyStatus());
	wiringPiI2CReadBytes(fd, HMC5883L_ADDRESS, HMC5883L_RA_DATAX_H, 6, buffer);
...
bool HMC5883L::getLockStatus() {
	wiringPiI2CReadBit(fd, HMC5883L_ADDRESS, HMC5883L_RA_STATUS, HMC5883L_STATUS_LOCK_BIT, buffer);
    return buffer[0];
}
...
bool HMC5883L::getReadyStatus() {
	wiringPiI2CReadBit(fd, HMC5883L_ADDRESS, HMC5883L_RA_STATUS, HMC5883L_STATUS_READY_BIT, buffer);
    return buffer[0];
}
...
uint8_t HMC5883L::getIDA() {
	wiringPiI2CReadByte(fd, HMC5883L_ADDRESS, HMC5883L_RA_ID_A, buffer);
    return buffer[0];
}
...
uint8_t HMC5883L::getIDB() {
	wiringPiI2CReadByte(fd, HMC5883L_ADDRESS, HMC5883L_RA_ID_B, buffer);
    return buffer[0];
}
...
uint8_t HMC5883L::getIDC() {
	wiringPiI2CReadByte(fd, HMC5883L_ADDRESS, HMC5883L_RA_ID_C, buffer);
    return buffer[0];
}
...
void HMC5883L::getYaw(int16_t mx, int16_t my, float *yaw)
{
	float tmp = atan2(mx, my) * 180 / M_PI;
	*yaw = tmp - Heading_offset;
}
